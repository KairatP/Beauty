//
//  GiftViewModel.swift
//  WoltK
//
//  Created by Kairat on 9/8/19.
//  Copyright © 2019 Kairat Parmanov. All rights reserved.
//

import Foundation

class GiftViewModel {
    
    let headerImageName = "14"
    
    let model: [GiftModel] = [
        GiftModel(sertificateName: "Econom:", sertifcatePrice: "500"),
        GiftModel(sertificateName: "Bronze:", sertifcatePrice: "10 000"),
        GiftModel(sertificateName: "Gold:", sertifcatePrice: "15 000")
    ]
    
    let giftExplanation = "asvhbakljdbcjkadsnc ac as cas cl ascas cas cpaslcaslc acas c as casj cp ascasc has c as cas lchasch ascahjsc hasc"
}
